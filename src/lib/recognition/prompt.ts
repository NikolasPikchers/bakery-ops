...
export function buildRecognitionPrompt(catalog: CatalogEntry[], sheetType: SheetType): string {
  ...
  return [
    ...
    '',
    'Каталог позиций пекарни — сверяй наименования строк с этим списком (в нём и пироги/выпечка, и десерты/кондитерка; на одном листе бывают и те, и другие):',
    list,
    '',
    'Правила:',
    ...
    '',
    'Даты на листе в русском формате ДД.ММ (день.месяц), напр. «5.06» = 5 июня, «6.06» = 6 июня —',
    'трактуй ПЕРВОЕ число как день, второе как месяц. Возвращай ISO YYYY-MM-DD; год текущий, если не указан.',
  ].join('\n');
}
